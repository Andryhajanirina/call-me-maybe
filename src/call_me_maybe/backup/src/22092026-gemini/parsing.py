"""Pipeline for prompt processing and constrained JSON output generation."""
...
class GenerationPipeline:
    """Manages token generation using constrained decoding."""
    ...
    def _build_prompt(self, user_prompt: str) -> str:
        """Construct system prompt with function descriptions and exact JSON template."""
        functions_desc = []
        ...
        return (
            ...
            f"Generate a JSON function call in this exact format:\n"
            f'{{"name": "FUNCTION_NAME", "parameters": {{...}}}}\n\n'
            f'JSON Output:\n{{"name": "'
        )

    def process_prompt(self, prompt: str) -> FunctionCallResult:
        """Process a prompt and return a structured FunctionCallResult."""
        ...
        for _ in range(self.max_tokens):
            ...
            if constraints.decoder.state.name == "DONE":
                break

        full_json = constraints.generated_text

        # Nettoyage et complétion basique pour le parsing
        ...
        # Sécurité absolue : garantir que fn_params est toujours un dictionnaire
